...
async function handler(request: NextRequest, props: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    const strapiUrl = `${process.env.NEXT_PUBLIC_STRAPI_URL || 'http://localhost:1337'}/api/${pathString}${searchParams}`;
    ...
    return new NextResponse(data, {
      ...
      headers: {
        'Content-Type': strapiRes.headers.get('Content-Type') || 'application/json',
      },
    });
